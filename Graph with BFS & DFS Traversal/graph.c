 ...
 * iterative Depth-First Search (DFS) starting at the source.
 * param graph	The graph structure being searched.
 * param source		Search FROM here.
 * param destination	Search TO here.
 * return 1 if there is a path, 0 otherwise.
 */
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination) {
	...
	while (!dequeIsEmpty(aStack)) {
		...
		int i;
		for (i = 0; i < cur->numNeighbors; i++) {			// Check cur's neighbors.
			if (!(cur->neighbors[i]->isVisited))
				dequePushBack(aStack, cur->neighbors[i]);	// Add unvisted to top of stack.
		}
	}
	dequeDelete(aStack);
	return 0;
...
/* Determines if there is a path from the source to the destination using an
 * iterative Breadth-First Search (BST) starting at the source.
 * param graph	The graph structure being searched.
 * param source		Search FROM here.
 * param destination	Search TO here.
 * return 1 if there is a path, 0 otherwise.
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination) {
	...
	while (!dequeIsEmpty(aQueue)) {
		...
		int i;
		for (i = 0; i < cur->numNeighbors; i++) {			// Check cur's neghbors.
			if (!(cur->neighbors[i]->isVisited))
				dequePushBack(aQueue, cur->neighbors[i]);	// Add unvisited to back of Queue.
		}
	}
	dequeDelete(aQueue);
	return 0;
 ...
 * connecting random pairs of vertices. The edges are unique, and thus there is
...
Graph* loadGraph(const char* fileName) {
    ...
    while (fgets(buffer, sizeof buffer, file) != NULL) {
		...
		// Create edges
		for (int i = (int) strtol(begin, &end, 10); end != begin; i = (int) strtol(begin, &end, 10)) {
